  ...
  run: async (client, interaction) => {
    if (
      ...
      return interaction.reply({
        content: "❌ | Yeterli yetkiye sahip değilsiniz!",
        ephemeral: true,
      });
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        "Aşağıdaki seçim menüsünü kullanarak özel komutlarını yönet."
      )
      .setColor("#ff0000")
...
client.on("interactionCreate", async (interaction) => {
  if (
    ...
    interaction.customId === "custom-command-menu"
  ) {
    if (
      !interaction.member.permissions.has(
        PermissionsBitField.Flags.ManageChannels
    ...
    if (selectedValue === "add_custom_command") {
      ...
      const commands = Object.entries(db.all()).filter(([key, value]) =>
        key.startsWith(`custom_command_${interaction.guild.id}_`)
      );
      const commandList =
        commands
          .map(
            ([key, entry]) =>
              `**${key.replace(
                `custom_command_${interaction.guild.id}_`,
                ""
              )}**: ${entry}`
          )
    ...
    if (interaction.customId === "addCustomCommandModal") {
      ...
      db.set(
        `custom_command_${interaction.guild.id}_${commandName}`,
        commandText
      );

      await interaction.reply({
        content: `✅ | **${commandName}** adlı özel komut başarıyla eklendi.`,
        ephemeral: true,
      });
      ...
      await interaction.reply({
        content: `✅ | **${commandName}** adlı özel komut başarıyla silindi.`,
        ephemeral: true,
      });
    }
  }
...
client.on("messageCreate", async (message) => {
  ...
  const commandName = message.content
    ...
    .shift()
    .toLowerCase();
  const commandText = db.fetch(
    `custom_command_${message.guild.id}_${commandName}`
  );
